...
RUN echo 'APT::Get::Assume-Yes "true";' > /etc/apt/apt.conf.d/andresilva85 \
  && echo 'DPkg::Options "--force-confnew";' >> /etc/apt/apt.conf.d/andresilva85

# Make sure PATH includes ~/.local/bin
# https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=839155
# This only works for root. The andresilva user is done near the end of this Dockerfile
...
RUN groupadd --gid 3434 andresilva \
  && useradd --uid 3434 --gid andresilva --shell /bin/bash --create-home andresilva \
  && echo 'andresilva ALL=NOPASSWD: ALL' >> /etc/sudoers.d/50-andresilva \
  && echo 'Defaults    env_keep += "DEBIAN_FRONTEND"' >> /etc/sudoers.d/env_keep

USER andresilva
ENV PATH /home/andresilva/.local/bin:/home/andresilva/bin:${PATH}
...
LABEL io.andresilva.preserve-entrypoint=true
